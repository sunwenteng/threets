...
export function loadAllRobot(callback:(err) => void):void {
    ...
    DB.Arena.fetchRobotRange(1, Enum.VALID_ROLE_ID, (err, result:any[]) => {
        if (err)
            return callback(err);
...
export function saveAllUpdateRobot(callback:(err)=>void):void {
    ...
    async.eachLimit(robotList, 20, (robotId, cb) => {
        ...
        PlayerInfoMgr.updateBasic(robot.accountId, robot.buildCacheBasic(), ()=>{});
        PlayerInfoMgr.updateMainHero(robot.accountId, robot.buildCacheMainHero(), ()=>{});

        DB.Arena.insertOrUpdateRobot(content, (err) => {
            finished += 1;
